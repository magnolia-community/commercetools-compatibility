...
public class PricesListField extends AbstractCustomMultiField<PricesListFieldFactory.Definition, PropertysetItem> {
    ...
    @Inject
    public PricesListField(PricesListFieldFactory.Definition definition, FieldFactoryFactory fieldFactoryFactory, ComponentProvider componentProvider, Item relatedFieldItem, I18NAuthoringSupport i18nAuthoringSupport) {
    ...
    protected void initFields(PropertysetItem fieldValues) {
        ...
        for (Price price : ((ProductVariant)((BeanItem)relatedFieldItem).getBean()).getPrices()) {
            AbstractOrderedLayout categoryFieldRoot = new HorizontalLayout();

            for (ConfiguredFieldDefinition fieldDefinition : definition.getFields()) {
                Field<?> categoryField = createLocalField(fieldDefinition, price, false);
     ...
     */
    protected Field<?> createLocalField(FieldDefinition fieldDefinition, Price price, boolean setCaptionToNull) {
        FieldFactory fieldfactory = fieldFactoryFactory.createFieldFactory(fieldDefinition, new BeanItem(price));
